...
export function AuthScreen({ onConnected }: Props) {
  ...
  return (
    <div style={styles.container}>
      ...
      <p style={styles.desc}>Connect your Google account to search your contacts and fill in details automatically.</p>
      {error && <p style={styles.error}>{error}</p>}
      <button style={styles.btn} onClick={handleConnect} disabled={loading}>
        {loading ? 'Opening sign-in...' : 'Connect Google Account'}
      </button>
      {loading && <p style={styles.hint}>A sign-in window should have opened. Complete sign-in there.</p>}
    </div>
  );
}

const styles: Record<string, React.CSSProperties> = {
  container: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 24px', textAlign: 'center', height: '100%', gap: '12px' },
  icon: { fontSize: '48px' },
  ...
  btn: { marginTop: '8px', padding: '10px 20px', background: '#4285F4', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 500, cursor: 'pointer', width: '100%', maxWidth: '260px' },
  hint: { fontSize: '12px', color: '#999', margin: 0 },
  error: { fontSize: '12px', color: '#d32f2f', background: '#fde8e8', padding: '8px 12px', borderRadius: '6px', margin: 0, maxWidth: '260px' },
};
