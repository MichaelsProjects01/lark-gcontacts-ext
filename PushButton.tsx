...
export function PushButton() {
  ...
  return (
    <div style={styles.wrapper}>
      ...
      <textarea style={styles.textarea} placeholder="Add context (pain points, what they need, how you know them)..." value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} />
      <button style={styles.btn} onClick={handlePush} disabled={loading}>
        {loading ? 'Sending...' : '🚀 Push to SolusLink CRM'}
      </button>
      {status && (
        <div style={{ ...styles.status, color: status.ok ? '#2e7d32' : '#c62828', background: status.ok ? '#e8f5e9' : '#fde8e8' }}>
          {status.msg}
        </div>
      )}
    </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  label: { margin: 0, fontSize: '12px', color: '#555', lineHeight: 1.4 },
  textarea: { resize: 'vertical', border: '1.5px solid #e0e0e0', borderRadius: '8px', padding: '8px 10px', fontSize: '12px', fontFamily: 'inherit', color: '#1a1a1a', outline: 'none', lineHeight: 1.4 },
  btn: { padding: '10px', background: '#1a73e8', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', width: '100%' },
  status: { padding: '8px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 500 },
};
